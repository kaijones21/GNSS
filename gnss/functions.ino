void setupGNSS(SFE_UBLOX_GPS &gnss){
  ...
  int minSurveyTime = 300;     // minimum amount of time [sec] for Survey-In to do
  ...
  if(gnss.svin.active == true){
    Serial.print(F("Survey is already in progress.\n"));
  }
  else{
    ...
    Serial.print(F("Survey has begun. Minimum time: "));
    Serial.print(minSurveyTime);
    Serial.print(F(".\tMinimum Accuracy: "));
